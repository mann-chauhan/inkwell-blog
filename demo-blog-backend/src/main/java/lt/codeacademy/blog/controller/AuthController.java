...
public class AuthController {
    ...
    public ResponseEntity<JsonNode> login(@Valid @RequestBody AuthRequest loginRequest) {
        return authService.login(loginRequest);
    }

    @PostMapping(value = "/signup", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<JsonNode> signup(@Valid @RequestBody AuthRequest authRequest) {
        return authService.signup(authRequest)
                ? ResponseEntity.ok(responseMessage(authRequest.getUserName()))
                : ResponseEntity.badRequest().body(responseMessage(authRequest.getUserName()));
    }
